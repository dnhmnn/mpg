...
export default function Hub() {
  ...
  return (
    <div style={{ '--bg-card': 'rgba(107,15,26,0.06)', '--bg-subtle': 'rgba(107,15,26,0.03)', '--border': 'rgba(107,15,26,0.12)', '--border-medium': 'rgba(107,15,26,0.15)', '--shadow-sm': '0 2px 16px rgba(107,15,26,0.08)' } as React.CSSProperties}>
      ...
      {/* Kurzbefehle Handle — sits above the dock */}
      <div
        style={{ position: 'fixed', bottom: 'calc(88px + env(safe-area-inset-bottom))', left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, cursor: 'pointer', userSelect: 'none', WebkitUserSelect: 'none', zIndex: 399 }}
        onClick={() => setSheetOpen(true)}
        onTouchStart={e => { touchStartY.current = e.touches[0].clientY }}
        onTouchEnd={e => { if (touchStartY.current - e.changedTouches[0].clientY > 30) setSheetOpen(true) }}
      >
        <span style={{ fontSize: 10, fontWeight: 600, color: 'rgba(0,0,0,0.28)', letterSpacing: '.04em', textTransform: 'uppercase' }}>Kurzbefehle</span>
        <div style={{ width: 32, height: 4, borderRadius: 99, background: 'rgba(0,0,0,0.15)' }} />
      </div>
