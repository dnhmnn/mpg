...
// ── Drug search via openFDA (free, no key, international coverage) ────────────
async function searchFda(q: string): Promise<Hit[]> {
  try {
    ...
    for (const item of d.results || []) {
      ...
      const wirkstoff = substances.map((s: string) =>
...
function extractPzn(raw: string): string | null {
  // Code-39 format: "PZN-1234567" / "PZN 1234567" / just digits
...
const lbl: React.CSSProperties = {
  display: 'block', fontWeight: 700, fontSize: '.88rem', color: '#111827', marginBottom: 3,
}
...
export default function DauermedikationPicker({
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (hitsRef.current && !hitsRef.current.contains(e.target as Node)) setShowHits(false)
    }
  ...
  function resetForm() {
    setAdding(false); setName(''); setWirkstoff(''); setDosis(''); setPzn('')
    setQuery(''); setHits([]); setScanMsg(''); setScanError(''); stopScan()
  }
  ...
  async function startScan() {
    ...
    if (!('mediaDevices' in navigator)) {
      setScanError('Kamera nicht verfügbar auf diesem Gerät.')
      return
    }
    try {
      ...
      if (!BD) {
        setScanMsg('Barcode-Erkennung nicht verfügbar (nur Chrome/Edge). Bitte PZN manuell eingeben.')
        return
      }
      const detector = new BD({ formats: ['code_39', 'code_128', 'data_matrix', 'ean_13', 'ean_8', 'qr_code'] })
      let found = false
      ...
      const tick = async () => {
        ...
        try {
          ...
          if (codes.length > 0) {
            ...
            if (p) {
              setPzn(p)
              setScanMsg(`✓ PZN ${p} erkannt. Bitte Medikamentname und Wirkstoff eintragen.`)
              // Use PZN as search hint
              setQuery(p)
              setSearching(true)
              ...
              setScanMsg(`Code erkannt: ${raw}. Bitte Wirkstoff manuell eintragen.`)
              setName(raw)
            }
            stopScan()
            return
      ...
      setScanError('Kamera-Fehler: ' + (e.message || 'Zugriff verweigert'))
    }
  }
          ...
          {value.map((m, i) => (
            <div key={i} style={{
              ...
              <div>
                ...
                {m.pzn && <span style={{ color: '#b45309', marginLeft: 5, fontSize: 11 }}>PZN {m.pzn}</span>}
              </div>
              <button
                type="button"
      ...
      {adding ? (
        <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: '1rem' }}>
          ...
          <div style={{ marginBottom: '.75rem', position: 'relative' }} ref={hitsRef}>
            <label style={lbl}>🔍 In Medikamentendatenbank suchen</label>
            <div style={{ display: 'flex', gap: 8 }}>
              <div style={{ position: 'relative', flex: 1 }}>
                <input
                  ...
                  placeholder="Medikamentname oder PZN eingeben…"
                  autoComplete="off"
                />
                {searching && (
                  <div style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', fontSize: 11, color: '#6b7280' }}>sucht…</div>
                )}
              </div>
              <button
                type="button"
            ...
            {showHits && hits.length > 0 && (
              <div style={{
                ...
                <div style={{ padding: '6px 12px 4px', fontSize: 11, fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '.04em', background: '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
                  Ergebnisse – Wirkstoff übernehmen
                </div>
                {hits.map((h, i) => (
                  <button
                    key={i}
                  ...
                  >
                    ...
                    <div style={{ fontSize: 12, color: '#6b7280', marginTop: 1 }}>Wirkstoff: <strong style={{ color: '#374151' }}>{h.wirkstoff}</strong></div>
                  </button>
                ))}
              </div>
            )}
          ...
          {scanning && (
            <div style={{ marginBottom: '.75rem' }}>
              ...
              <div style={{ fontSize: 13, color: '#374151', marginTop: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                Kamera aktiv – Barcode/PZN-Aufkleber vor die Kamera halten
              </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(175px,1fr))', gap: '.6rem', marginBottom: '.75rem' }}>
            <div>
              <label style={lbl}>Medikamentname *</label>
              <input style={inp} type="text" value={name} onChange={e => setName(e.target.value)} placeholder="z.B. Novalgin" />
            </div>
            <div>
              <label style={lbl}>Wirkstoff *</label>
              <input style={inp} type="text" value={wirkstoff} onChange={e => setWirkstoff(e.target.value)} placeholder="z.B. Metamizol-Natrium" />
            </div>
            <div>
              <label style={lbl}>Dosis / Einnahme</label>
              <input style={inp} type="text" value={dosis} onChange={e => setDosis(e.target.value)} placeholder="z.B. 500 mg 3×/Tag" />
            </div>
            <div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button type="button" onClick={resetForm} style={btn('#6b7280')}>
              Abbrechen
            </button>
          </div>
        </div>
