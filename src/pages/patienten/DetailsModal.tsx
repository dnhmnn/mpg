...
function F({ label, value, ch }: { label: string; value?: string | number | boolean | null; ch?: boolean }) {
  if (!value && value !== 0) return null
  return (
    <div style={{ marginBottom: 10, ...(ch ? { borderLeft: '3px solid #d97706', paddingLeft: 8, marginLeft: -8 } : {}) }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: ch ? '#d97706' : 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>{label}</div>
      <div style={{ fontSize: 15, color: 'var(--text)', whiteSpace: 'pre-wrap' }}>{String(value)}</div>
    </div>
  )
}
...
function Tag({ active, label }: { active?: boolean; label: string }) {
  return (
    <span style={{
      ...
      border: `0.5px solid ${active ? 'transparent' : 'var(--border-medium)'}`,
...
function CatSection({ label, items }: { label: string; items: [boolean | undefined, string][] }) {
  return (
    <div style={{ borderTop: '0.5px solid var(--border)', paddingTop: 6, marginTop: 6 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>{label}</div>
      <Tags items={items} />
    </div>
  )
}
  ...
  function printDoc() {
    ...
    if (type === 'patient') {
      ...
      <div style="display:grid;grid-template-columns:1fr 80pt;gap:1.5pt;margin-bottom:1.5pt">
        ${blk('Einsatzdaten',
          row('1fr 1.2fr 1fr 1fr 1fr 1fr',
            cell('Einsatz-Nr.',p.einsatz_nr||'','einsatz_nr')+cell('Auftrags-Nr. (ILS)',p.auftrags_nr||'','auftrags_nr')+cell('Datum',new Date().toLocaleDateString('de-DE'))+cell('Alarmzeit',p.zeit_einsatz||'','zeit_einsatz')+cell('Eintreffen',p.zeit_eintreffen||'','zeit_eintreffen')+cell('Transportbeginn',p.zeit_transport||'','zeit_transport')
          )+row('1fr 1fr 1fr 2fr',
            cell('Übergabe',p.zeit_uebergabe||'','zeit_uebergabe')+cell('Rufname',p.rufname||'','rufname')+cell('Fahrzeug / Einheit',p.fahrzeug||'','fahrzeug')+cell('Einsatzart / Stichwort',p.einsatz_art||'','einsatz_art')
          )+row('2fr 1fr',
            cell('Einsatzort / Adresse',p.einsatz_adresse||'','einsatz_adresse')+cell('Transportziel',p.transport_ziel||'','transport_ziel')
          )
        )}
        ${blk('Mannschaft',
          row('1fr',cell('Teamführer',  (p as any).mannschaft?.tf?.name || p.mannschaft_tf||''))+
          row('1fr',cell('Mannschaft 1',(p as any).mannschaft?.m1?.name || p.mannschaft_1||''))+
          row('1fr',cell('Mannschaft 2',(p as any).mannschaft?.m2?.name || p.mannschaft_2||''))+
          row('1fr',cell('Mannschaft 3',(p as any).mannschaft?.m3?.name || p.mannschaft_3||''))
        )}
      </div>

      ${blk('Patientenstammdaten',
        row('2fr 1fr .6fr',
          cell('Name, Vorname',[p.name,p.vorname].filter(Boolean).join(', '))+cell('Geburtsdatum',p.gebdatum||'','gebdatum')+cell('Alter',p.alter||'','alter')
        )+row('2fr 1fr 1fr',
          cell('Straße',p.strasse||'','strasse')+cell('PLZ / Ort',p.plz_ort||'','plz_ort')+cell('Telefon',p.telefon||p.mobil||'')
        )+row('1.5fr 1fr 1.5fr 1.5fr',
          cell('Krankenkasse',p.kasse||'','kasse')+cell('Vers.-Nr.',p.versnr||'','versnr')+cell('Hausarzt',p.hausarzt||'','hausarzt')+cell('Angehöriger / Kontakt',p.angehoeriger||'','angehoeriger')
        )
      )}

      ${blk('Notfallgeschehen / Anamnese',
        row('1fr',cellL('Notfallgeschehen',p.notfallgeschehen||'','notfallgeschehen'))+
        row('1fr',cellL('Verlaufsbeschreibung',p.verlaufsbeschreibung||'','verlaufsbeschreibung'))+
        row('1fr 1fr',cellL('Vorerkrankungen',p.vorerkrankungen||'','vorerkrankungen')+cellL('Dauermedikation Patient',p.vormedikation_patient||'','vormedikation_patient'))+
        row('1fr',cell('Allergien / Unverträglichkeiten',p.allergien||'Keine bekannt','allergien'))
      )}
        ...
        return `<div style="border:0.5pt solid #888;margin-bottom:2pt;overflow:hidden">
          ...
          <div style="display:flex">
            ...
            <div style="flex:1;min-width:0;overflow:hidden">
              ...
              <div style="height:8pt;border-bottom:0.5pt solid #ccc;display:grid;grid-template-columns:repeat(${cols},1fr)">${bottomCells('o2' as 'spo2')}</div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1pt;margin-bottom:2pt">
        ...
        <div>
          ${blk('Reanimation',cr([[p.rean,'CPR durchgeführt'],[p.rean_tod,'Todesfeststellung']])+
            (p.rean_tod&&p.rean_tod_zeit?row('1fr',cell('Uhrzeit Todesfeststellung',p.rean_tod_zeit,'rean_tod_zeit')):'')+
            (p.rean?row('1fr 1fr 1fr',cell('Beginn',p.rean_beginn||'','rean_beginn')+cell('Ende',p.rean_ende||'','rean_ende')+cell('Defibrillationen',p.rean_defib||'','rean_defib')):'')
          )}
          ${blk('Venöser Zugang',row('1fr 1fr 1fr',cell('Art',p.zugang_art||'','zugang_art')+cell('Gauge',p.zugang_gauge||'','zugang_gauge')+cell('Region',p.zugang_region||'','zugang_region')))}
          ${blk('Infusion',row('1fr 1fr',cell('Art',p.inf_art||'','inf_art')+cell('Menge (ml)',p.inf_menge||'','inf_menge')))}
        </div>
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1pt;margin-bottom:2pt">
        ${blk('Beatmung',cr([[p.beat_manuell,'Manuell'],[p.beat_maschinell,'Maschinell'],[p.beat_niv,'NIV'],[p.beat_notfallnarkose,'Notfallnarkose']])+
          row('1fr 1fr 1fr 1fr 1fr',cell('FiO₂',p.beat_fio2||'','beat_fio2')+cell('AF /min',p.beat_af||'','beat_af')+cell('PEEP mbar',p.beat_peep||'','beat_peep')+cell('Pmax mbar',p.beat_pmax||'','beat_pmax')+cell('AMV l/min',p.beat_amv||'','beat_amv'))
        )}
        ${blk('Defibrillation',cr([[p.defi_aed,'AED'],[p.defi_defi,'Defi'],[p.defi_mono,'Monophasisch'],[p.defi_bi,'Biphasisch']])+
          ...
          row('1fr 1fr 1fr 1fr',cell('Zeitpunkt 1. Defi',p.defi_zeitpunkt||'','defi_zeitpunkt')+cell('ROSC',p.defi_rosc||'','defi_rosc')+cell('Anzahl',p.defi_anzahl||'','defi_anzahl')+cell('Energie (kJ)',p.defi_energie||'','defi_energie'))
        )}
      </div>
      ...
      ${blk('Übergabe / Besonderheiten',
        row('1fr 1fr',cell('Übergabe Ziel',p.uebergabe_ziel||'','uebergabe_ziel')+cell('Übergabe an (Name)',p.uebergabe_name||'','uebergabe_name'))+
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'var(--bg)', zIndex: 2000, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
      ...
      <header style={{ position: 'sticky', top: 0, background: 'var(--bg-status-bar)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderBottom: '0.5px solid var(--border)', zIndex: 10, flexShrink: 0 }}>
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 1rem', height: 54, display: 'flex', alignItems: 'center', gap: 8 }}>
          <button onClick={onClose} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'transparent', border: 'none', color: 'var(--accent)', fontWeight: 600, fontSize: 16, cursor: 'pointer', padding: '8px 0', fontFamily: 'inherit', flexShrink: 0 }}>
            {pik(<path d="M15 18l-6-6 6-6"/>)}
            Zurück
          </button>
          ...
          <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
            ...
            <button onClick={printDoc} title="Drucken / PDF" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: '0.5px solid var(--border)', borderRadius: 8, color: 'var(--text)', cursor: 'pointer', padding: '8px 10px', fontFamily: 'inherit' }}>
              {pik(<><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></>, 18)}
            </button>
          </div>
        </div>
      </header>
      ...
      {/* Content */}
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '1rem 1rem 80px', width: '100%' }}>
        ...
        {!isPatient && nd && (
          <>
            <PubSection title="Einsatz" open icon={pik(<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>)}>
              <Grid>
                <F label="Stichwort" value={nd.stichwort} />
                <F label="Kategorie" value={nd.kategorie} />
                <F label="Alarmzeit" value={nd.datum_alarmzeit} />
                <F label="Einsatzende" value={nd.datum_einsatzende} />
                <F label="ILS-Nummer" value={nd.einsatznummer_ils} />
                <F label="Adresse" value={nd.adresse} />
                <F label="Meldebild" value={nd.meldebild} />
                <F label="Disponierte EM/FW" value={nd.disponierte_em_fw} />
                <F label="Disponierte EM/RD" value={nd.disponierte_em_rd} />
              </Grid>
            </PubSection>
            <PubSection title="Patient" icon={pik(<><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></>)}>
              <Grid>
                <F label="Patienten-Daten erhoben" value={nd.patienten_daten_erhoben ? 'Ja' : 'Nein'} />
                <F label="Name" value={nd.patient_name} />
                <F label="Alter / Geburtsdatum" value={nd.patient_alter_geburtsdatum} />
                <F label="ILS Patientennummer" value={nd.patient_nummer_ils} />
              </Grid>
            </PubSection>
            <PubSection title="Sachverhalt" open icon={pik(<><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1" ry="1"/></>)}>
              <F label="Sachverhalt" value={nd.sachverhalt} />
              <F label="Protokollpflichtig" value={nd.protokollpflichtig ? 'Ja' : 'Nein'} />
              <F label="Begründung" value={nd.protokollpflichtig_begruendung} />
            </PubSection>
            <PubSection title="Verantwortlicher" icon={pik(<><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></>)}>
              <Grid>
                <F label="Unterwiesen" value={nd.verantwortlicher_unterwiesen ? 'Ja' : 'Nein'} />
                <F label="Name" value={nd.verantwortlicher_name} />
                <F label="Qualifikation" value={nd.verantwortlicher_qualifikation} />
              </Grid>
            </PubSection>
            <PubSection title="Nacherfasst von" open icon={pik(<><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></>)}>
              <Grid>
                <F label="Name" value={nd.nacherfasst_von_name} />
                <F label="Qualifikation" value={nd.nacherfasst_von_qualifikation} />
                <F label="Datum" value={fmtDateTime(nd.nacherfasst_datum)} />
              </Grid>
              {nd.nacherfasst_unterschrift && (
                <img src={nd.nacherfasst_unterschrift} alt="Unterschrift" style={{ maxWidth: '100%', maxHeight: 120, border: '0.5px solid var(--border)', borderRadius: 8, background: '#fff', marginTop: 8 }} />
              )}
            </PubSection>
          </>
        )}
