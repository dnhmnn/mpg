...
export default function DetailsModal({ doc, type, onClose }: Props) {
  function printDoc() {
    ...
    if (type === 'patient') {
      ...
      const cell = (lbl:string, val:string) => `<div style="padding:1pt 3pt;border-right:0.5pt solid #ccc"><div style="font-size:4pt;color:#666;text-transform:uppercase;letter-spacing:.3pt;margin-bottom:0.5pt">${lbl}</div><div style="font-weight:bold;font-size:6.2pt;min-height:7pt">${val||''}</div></div>`
      const cellL = (lbl:string, val:string) => `<div style="padding:1pt 3pt;border-right:0.5pt solid #ccc"><div style="font-size:4pt;color:#666;text-transform:uppercase;letter-spacing:.3pt;margin-bottom:0.5pt">${lbl}</div><div style="font-size:6.2pt;min-height:9pt;white-space:pre-wrap">${val||''}</div></div>`
      const naca = ['0','I','II','III','IV','V','VI','VII'].map(v=>`<span class="tag naca${p.naca===v?' active':''}">${v}</span>`).join('')
      const cr = (items:[boolean|undefined,string][]) => `<div style="display:flex;flex-wrap:wrap;gap:1pt;padding:1pt 3pt">${items.map(([on,l])=>tag(on,l)).join('')}</div>`
      const catRow = (cat:string, items:[boolean|undefined,string][]) => `<div style="display:flex;flex-wrap:wrap;gap:0.5pt;padding:0.5pt 3pt;border-top:0.5pt solid #e0e0e0;align-items:center"><span style="font-size:4pt;font-weight:bold;color:#444;text-transform:uppercase;min-width:30pt;flex-shrink:0">${cat}</span>${items.map(([on,l])=>tag(on,l)).join('')}</div>`
      ...
      <style>
        *{box-sizing:border-box;margin:0;padding:0}
        body{font-family:Arial,Helvetica,sans-serif;font-size:6.2pt;color:#000;background:#fff}
      ...
      <div style="display:grid;grid-template-columns:1fr 80pt;gap:1.5pt;margin-bottom:2pt">
        ${blk('Einsatzdaten',
